...
function App() {
  ...
  const fetchCarsHandler = useFetchCars(url, setCars, setLoading, setLoadedCount, setIsError)
  ...
  const standardDeviation = calcStandardDeviation(cars.map(item => item.priceUSD))
  ...
  return (
    <div className="App">
      {!url &&
        <p className='message'>Плагін оптимізований для пошуку на сайті <a href="https://auto.ria.com/uk/">Auto.RIA</a>.
        </p>}
      ...
      {isCanViewChart && mode !== MODE[3] && <MainChart
        data={data}
        colors={colors}
        scale={scale}
        average={average}
      ...
      {isCanViewChart && mode !== MODE[3] && <InfoBlock
        median={median}
        average={average}
      ...
      {isHelpText &&
        <p className='message'>Щоб користуватися аналітикою, перейдіть на <a href='https://auto.ria.com/uk/advanced-search/?indexName=auto&price.currency=1&abroad.not=0&custom.not=1&size=100'>сторінку пошуку</a>, встановіть потрібні фільтри,
          і коли будете на сторінці видачі - кнопку завантаження у цьому вікні.</p>}
      {isCanViewLoadingButton && <LoadingButton onClick={fetchCarsHandler} disabled={loading}/>}
      <Loader loading={loading} counter={loadedCount}/>
      {isCanViewLoadingButton && isError && <p className='message error'>
