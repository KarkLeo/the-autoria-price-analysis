  ...
  return (<div className='head-toolset'>
    <div className='head-toolset__item'>
      ...
      <div className='head-toolset__buttons-set'>
        {MODE.map((item, index) =>
          <button className='head-toolset__button-item' key={item} onClick={() => setMode(item)}
                  disabled={item === mode}>{ModeTranslate[item]}</button>)}
      </div>
    </div>
    {mode !== MODE[3] && <div className='head-toolset__item'>
