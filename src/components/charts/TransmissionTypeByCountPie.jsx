...
import { createDatasetForChartByTransmissionType } from '../../tools/createDatasetForChart'
...
export const TransmissionTypeByCountPie = ({ data }) => {
  ...
  const transmissionTypeByCountColors = generateRandomColors(
    transmissionTypeByCount.length
  )
          ...
          <PieChart width={300} height={300}>
            ...
            >
              {transmissionTypeByCount.map((entry, index) => (
                <Cell
                  key={`cell-${entry.key}`}
                  fill={transmissionTypeByCountColors[index]}
                />
              ))}
            </Pie>
            <Tooltip />
