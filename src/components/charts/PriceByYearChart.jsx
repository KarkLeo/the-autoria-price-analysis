  ...
  return <div className='chart--small'>
    <ResponsiveContainer width="100%" height="100%" minWidth={480} minHeight={200}>
      <ComposedChart minWidth={480} minHeight={200} data={priceByYear}>
        ...
        <XAxis dataKey="year" type='number' unit='р' includeHidden domain={[`dataMin`, `dataMax`]}
               tickCount={priceByYear.length}/>
        <YAxis unit='$'/>
        <Tooltip/>
        <Area type='natural' dataKey="range" name='Розмах ціни' stroke="#fae100" fill="#fae100"/>
        <Line type="natural" dataKey="middle" name='Середня ціна' stroke="#b71414"/>
      </ComposedChart>
    </ResponsiveContainer>
  </div>
}